...
export function setAuthCookieHeaders(
  ...
  return {
    'Set-Cookie': [
      ...
      `${AUTH_CONSTANTS.COOKIE_REFRESH_TOKEN}=${refreshToken}; HttpOnly; Path=/; SameSite=Strict; Max-Age=${AUTH_CONSTANTS.REFRESH_TOKEN_MAX_AGE}${secure}`,
    ].join(', '),
  };
}
...
export function clearAuthCookieHeaders(): Record<string, string> {
  return {
    'Set-Cookie': [
      ...
      `${AUTH_CONSTANTS.COOKIE_REFRESH_TOKEN}=; HttpOnly; Path=/; SameSite=Strict; Max-Age=0`,
    ].join(', '),
  };
}
