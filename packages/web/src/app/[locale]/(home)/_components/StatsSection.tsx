      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 divide-x divide-border">
          <div className="flex flex-col items-center justify-center text-center px-4">
            ...
            <p className="text-[12px] text-muted uppercase tracking-wider font-semibold mt-1">{t('avgYield')}</p>
          </div>
          <div className="flex flex-col items-center justify-center text-center px-4">
            <Users className="w-5 h-5 text-primary-500 mb-2" />
            <h3 className="font-display text-2xl font-bold text-foreground">12.5k</h3>
            <p className="text-[12px] text-muted uppercase tracking-wider font-semibold mt-1">{t('investors')}</p>
          </div>
          <div className="flex flex-col items-center justify-center text-center px-4">
            <h3 className="font-display text-2xl font-bold text-foreground">€52M</h3>
            <p className="text-[12px] text-muted uppercase tracking-wider font-semibold mt-1">{t('funded')}</p>
          </div>
          <div className="flex flex-col items-center justify-center text-center px-4">
            <ShieldCheck className="w-5 h-5 text-primary-500 mb-2" />
            <h3 className="font-display text-lg font-bold text-foreground mt-1">{t('regulated')}</h3>
            <p className="text-[12px] text-muted uppercase tracking-wider font-semibold mt-1">{t('platform')}</p>
          </div>
        </div>
      </div>
    </div>
