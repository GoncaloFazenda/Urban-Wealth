...
export function ListingCard({ listing, isOwn, onBuy }: ListingCardProps) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card shadow-card hover:shadow-card-hover transition-all overflow-hidden group">
      {/* Clickable top section → property detail */}
      <Link href={`/properties/${listing.propertyId}`} className="block">
        {/* Property Image */}
        ...
        <div className="px-4 pt-4 pb-0">
          <h3 className="text-[15px] font-bold text-foreground truncate mb-0.5 group-hover:text-primary-500 transition-colors">
            {listing.propertyTitle}
          </h3>
          <p className="text-[12px] text-muted">{listing.propertyLocation}</p>
        </div>
      </Link>
