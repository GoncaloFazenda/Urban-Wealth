...
function UserDropdown({ user, onLogout }: { user: { fullName: string }; onLogout: () => void }) {
  const t = useTranslations('Navbar');
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1.5 w-48 rounded-xl border border-border bg-card shadow-elevated py-1.5 z-50 animate-fade-in">
          ...
          <button
            ...
            className="w-full text-left px-4 py-2 text-[13px] font-medium text-muted hover:text-foreground hover:bg-surface-hover transition-colors"
          >
            {t('logOut')}
          </button>
        </div>
...
function DropdownLink({ href, onClick, children }: { href: string; onClick: () => void; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className="block px-4 py-2 text-[13px] font-medium text-muted hover:text-foreground hover:bg-surface-hover transition-colors"
    >
      {children}
    </Link>
  );
